...
async function installReadyManualMonitorFixture(
  routeIds: readonly string[],
  options: { addressByRoute?: Record<string, string>; memoByRoute?: Record<string, string> } = {},
): Promise<ReadyManualMonitorFixture> {
  ...
  for (const route of routes) {
    let monitorNetworkId = monitorByCode.get(route.networkCode);
    if (!monitorNetworkId) {
      const [existing] = await db.select({ id: blockchainMonitorNetworksTable.id })
        .from(blockchainMonitorNetworksTable)
        .where(eq(blockchainMonitorNetworksTable.networkCode, route.networkCode))
        .limit(1);
      ...
      monitorNetworkIds.push(monitorNetworkId);
      monitorByCode.set(route.networkCode, monitorNetworkId);
    }
    const [asset] = await db.insert(blockchainMonitorAssetsTable).values({
      monitorNetworkId,
      ...
      enabled: true,
    }).returning({ id: blockchainMonitorAssetsTable.id });
    monitorAssetIds.push(asset!.id);
...
async function restoreReadyManualMonitorFixture(fixture: ReadyManualMonitorFixture) {
  const { cryptoAssetNetworksTable, db } = await import("@workspace/db");
  for (const route of fixture.routeSnapshots) {
    await db.update(cryptoAssetNetworksTable).set(route)
      .where(eq(cryptoAssetNetworksTable.id, route.id));
  }
  if (fixture.monitorAssetIds.length) {
    await db.delete(blockchainMonitorAssetsTable)
      .where(inArray(blockchainMonitorAssetsTable.id, fixture.monitorAssetIds));
  }
  if (fixture.monitorNetworkIds.length) {
    const networks = await db.select({ endpointSecretRef: blockchainMonitorNetworksTable.endpointSecretRef })
      .from(blockchainMonitorNetworksTable)
      .where(inArray(blockchainMonitorNetworksTable.id, fixture.monitorNetworkIds));
...
// Manual monitoring coverage runs in the dedicated monitoring/manual suites.
...
test("owner receiving-wallet updates validate, audit, and immediately gate exact asset-network rows", async () => {
  ...
  try {
    ...
    assert.equal(partiallyReadySelection.status, 200, JSON.stringify(partiallyReadySelection.body));
    const readyWithoutMemo = partiallyReadySelection.body.find(
      (row: { id: string }) => row.id === networkAId,
    );
    ...
    assert.equal(memoRequired?.monitoringReadiness?.ready, false);
    ...
    assert.equal(
      rowsAfterPartiallyReadySelection.find(row => row.id === networkAId)?.customerDepositsEnabled,
      true,
    );
    assert.equal(
      rowsAfterPartiallyReadySelection.find(row => row.id === networkBId)?.customerDepositsEnabled,
      false,
    );
    ...
    assert.equal(independentRows.find(row => row.id === networkBId)?.sharedDepositMemo, null);
    assert.equal(independentRows.find(row => row.id === networkBId)?.enabled, true);
    assert.equal(independentRows.find(row => row.id === networkBId)?.customerDepositsEnabled, false);
    ...
    assert.equal(invalidAddress.status, 200, JSON.stringify(invalidAddress.body));
    assert.equal(invalidAddress.body[0]?.customerDepositsEnabled, false);
    assert.equal(invalidAddress.body[0]?.monitoringReadiness?.code, "ADDRESS_INVALID");
    ...
    assert.equal(invalidOptionalMemo.status, 200, JSON.stringify(invalidOptionalMemo.body));
    assert.equal(invalidOptionalMemo.body[0]?.customerDepositsEnabled, false);
    assert.equal(invalidOptionalMemo.body[0]?.monitoringReadiness?.code, "MEMO_INVALID");
    assert.equal(invalidOptionalMemo.body[0]?.monitoringReadiness?.ready, false);
    ...
    const optionalMemoReady = optionalMemo.body.find(
      (row: { id: string }) => row.id === networkAId,
    );
    const optionalMemoRequired = optionalMemo.body.find(
      (row: { id: string }) => row.id === networkBId,
    );
    assert.equal(optionalMemoReady?.customerDepositsEnabled, true);
    assert.equal(optionalMemoReady?.monitoringReadiness?.code, "READY");
    assert.equal(optionalMemoRequired?.customerDepositsEnabled, false);
