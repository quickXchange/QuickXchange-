...
test("asset network bulk enablement preserves every network configuration", async () => {
  const {
    cryptoAssetNetworksTable,
    cryptoAssetsTable,
    db,
  ...
  const operatorAuth = await import("../src/lib/operator-auth");
  const suffix = randomUUID();
  const userId = `network-bulk-owner-${suffix}`;
  const assetId = `network-bulk-asset-${suffix}`;
  const networkIds = [
    `network-bulk-a-${suffix}`,
    `network-bulk-b-${suffix}`,
  ];
  const [operator] = await db.insert(operatorsTable).values({
    email: `network-bulk-owner-${suffix}@example.test`,
    clerkUserId: userId,
    role: "owner",
    status: "active",
  }).returning();
  await db.insert(cryptoAssetsTable).values({
    id: assetId,
    code: `NB${suffix.slice(0, 6)}`,
    name: "Network Bulk Asset",
    decimals: 8,
  });
  await db.insert(cryptoAssetNetworksTable).values([
    {
      id: networkIds[0],
      assetId,
      networkCode: `NBA-${suffix.slice(0, 8)}`,
      networkName: "Network Bulk A",
      decimals: 8,
      enabled: true,
      customerDepositsEnabled: true,
      depositProvider: "manual",
      sharedDepositAddress: "network-a-address",
      sharedDepositMemo: "network-a-memo",
      requiresMemo: true,
    },
    {
      id: networkIds[1],
      assetId,
      networkCode: `NBB-${suffix.slice(0, 8)}`,
      networkName: "Network Bulk B",
      decimals: 6,
      enabled: false,
      customerDepositsEnabled: false,
      depositProvider: "none",
      sharedDepositAddress: "network-b-address",
  ...
  const headers = { "x-test-clerk-user-id": userId };
  try {
    const before = await db.select().from(cryptoAssetNetworksTable)
      .where(inArray(cryptoAssetNetworksTable.id, networkIds));
    const disabled = await apiJson(
      api.url,
      `/admin/crypto-assets/${assetId}/networks/enabled`,
      { enabled: false },
      "PATCH",
      headers,
    );
    ...
    assert.ok(disabled.body.every((network: { enabled: boolean }) => !network.enabled));
    const afterDisable = await db.select().from(cryptoAssetNetworksTable)
      .where(inArray(cryptoAssetNetworksTable.id, networkIds));
    assert.ok(afterDisable.every(network => !network.enabled));
    ...
    assert.ok(enabled.body.every((network: { enabled: boolean }) => network.enabled));
    const afterEnable = await db.select().from(cryptoAssetNetworksTable)
      .where(inArray(cryptoAssetNetworksTable.id, networkIds));
    assert.ok(afterEnable.every(network => network.enabled));

    const config = (row: (typeof before)[number]) => ({
      id: row.id,
      customerDepositsEnabled: row.customerDepositsEnabled,
      depositProvider: row.depositProvider,
      sharedDepositAddress: row.sharedDepositAddress,
      sharedDepositMemo: row.sharedDepositMemo,
    ...
    assert.deepEqual(afterEnable.map(config).sort((a, b) => a.id.localeCompare(b.id)), before.map(config).sort((a, b) => a.id.localeCompare(b.id)));
  } finally {
    await db.delete(cryptoAssetNetworksTable)
      .where(inArray(cryptoAssetNetworksTable.id, networkIds));
    await db.delete(cryptoAssetsTable).where(eq(cryptoAssetsTable.id, assetId));
