...
import {
  ...
  ApplyWhitebitAutomaticRouteMappingsResponse,
...
import {
  isRegisteredDepositProvider,
  listDepositProviderOptions,
} from "../lib/deposit-provider-registry";
...
import {
  classifyWhitebitAutoMappings,
  isWhitebitRouteIdentityException,
  isWhitebitRouteMappingPlausible,
  plausibleWhitebitDepositNetworks,
  type WhitebitAutoMappingRoute,
} from "../lib/whitebit-auto-mapping";
import {
...
async function createOrderFromInput(
  ...
  const depositEligibilityContext = providerFundingCandidate
  ...
  const inserted = await db.transaction(async (tx) => {
    ...
    if (sourceSnapshot?.kind === "crypto-network") {
      ...
      if (providerFundingCandidate) {
        ...
        await assertCustomerDepositEligibilityContextCurrent(
          tx,
          depositEligibilityContext!,
        );
      }
      const [catalog] = await tx.select({
        ...
        .where(and(
          ...
          sql`upper(${cryptoAssetsTable.code}) = upper(${sourceSnapshot.assetCode.trim()})`,
        )).for("update").limit(1);
      const fundingSnapshot = manualFunding as (typeof manualFunding & {
        networkId?: string;
      ...
      const signedTrackingEnabled =
        fundingSnapshot?.manualWalletTrackingEnabled !== false;
      const requestedTrackingEnabled = (manualFunding as (typeof manualFunding & {
        manualWalletTrackingEnabled?: boolean;
      }) | undefined)?.manualWalletTrackingEnabled !== false;
      if (
        !catalog ||
        sourceSnapshot.networkId !== sourceRouteId ||
        fundingSnapshot?.networkId !== sourceRouteId ||
        ...
        catalog.route.manualWalletTrackingEnabled !== signedTrackingEnabled ||
        !catalog.asset.enabled ||
        catalog.asset.lifecycle === "deprecated" ||
        !catalog.route.enabled ||
        catalog.route.lifecycle === "deprecated" ||
        catalog.route.executionMode !== "manual" ||
        (providerFundingCandidate &&
          !isCustomerDepositEligible(
            catalog.asset,
            catalog.route,
        ...
        throw new ApiError(
          "SETTLEMENT_OPTION_CHANGED",
          "The selected crypto deposit route changed after this quote was issued. Refresh and try again.",
          409,
        );
      }
      if (
        selectedDepositProvider === "manual" &&
        (!isSyntacticallyValidManualWalletAddress(catalog.route, catalog.route.sharedDepositAddress) ||
        ...
        throw new ApiError(
          "DESK_CRYPTO_DEPOSIT_UNAVAILABLE",
          "The selected manual deposit wallet is no longer valid.",
          409,
        );
      }
      ...
      const catalogEligible = Boolean(
        ...
        route.depositProvider === selectedDepositProvider &&
    ...
    if (providerFundingCandidate && sourceSnapshot?.kind === "crypto-network" && manualFunding) {
      await tx.insert(whitebitOrderAddressesTable).values({
        ...
        providerTicker: (manualFunding.whitebitAssetCode ?? sourceSnapshot.assetCode).trim().toUpperCase(),
        network: (manualFunding.whitebitNetworkCode ?? sourceSnapshot.networkCode ?? quote.fromNetwork).trim().toUpperCase(),
        status: "claiming",
    ...
    if (
      ...
      isManualWalletTrackingEnabledForOrder(created)
    ) {
  ...
  if (!inserted) {
    ...
    if (
      isManualWalletTrackingEnabledForOrder(existing) &&
      (existing.fundingProviderSource === "manual" ||
        existing.providerState === "whitebit_fallback")
    ) {
      try {
  ...
  if (
    isManualWalletTrackingEnabledForOrder(inserted) &&
    inserted.fundingProviderSource === "manual"
  ) {
    try {
  ...
  if (providerFundingCandidate && sourceSnapshot?.kind === "crypto-network" && manualFunding) {
    ...
    const provisioned = await provisionSwapFundingAddress({
      ...
      whitebitAssetCode: manualFunding.whitebitAssetCode,
    ...
    if (finalOrder.providerState === "whitebit_fallback" &&
      isManualWalletTrackingEnabledForOrder(finalOrder) &&
...
function cryptoInput(body: unknown, network: boolean, update = false): JsonRecord {
  ...
  const allowed = network
    ? ["id","assetId","networkCode","networkName","logoObjectPath","networkFamily","decimals","executionMode","depositProvider","lifecycle","regions","enabled","customerDepositsEnabled","manualWalletTrackingEnabled","requiresMemo","requiredConfirmations","confirmationGuidance","explorerUrlTemplate","depositInstructions","depositWarning","sharedDepositAddress","sharedDepositMemo"]
  ...
  if (network && value.depositProvider !== undefined &&
      (typeof value.depositProvider !== "string" || !isRegisteredDepositProvider(value.depositProvider))) {
    throw new ApiError("VALIDATION_ERROR", "Invalid deposit provider.", 400);
  }
  ...
  for (const key of ["enabled","customerDepositsEnabled","manualWalletTrackingEnabled","requiresMemo"]) if (value[key] !== undefined && typeof value[key] !== "boolean") throw new ApiError("VALIDATION_ERROR", `Invalid ${key}.`, 400);
...
function widgetDepositReadiness(
  ...
  if (!isPlausibleWhitebitRouteSetup(
    capabilities, network.id, asset.code, network.networkCode,
    network.whitebitAssetCode, network.whitebitNetworkCode,
  ) || !matchWhitebitRouteCapability(capabilities, asset.code, network.networkCode,
    network.whitebitAssetCode, network.whitebitNetworkCode)) {
...
type DepositProviderAssignmentPair = {
  ...
  network: typeof cryptoAssetNetworksTable.$inferSelect;
  assetMapping: typeof whitebitAssetMappingsTable.$inferSelect | null;
};

type WhitebitRouteMappingSelection = { networkId: string; assetCode: string; networkCode: string };

function selectedWhitebitMappings(
  mappings: WhitebitRouteMappingSelection[] | undefined,
  ids: readonly string[],
  target: DepositProviderAssignmentTarget,
) {
  if (mappings?.length && target !== "whitebit") {
    throw new ApiError("VALIDATION_ERROR", "WhiteBIT mappings require the WhiteBIT deposit provider.", 400);
  }
  const selected = new Map<string, { assetCode: string; networkCode: string }>();
  for (const entry of mappings ?? []) {
    if (!ids.includes(entry.networkId) || selected.has(entry.networkId)) {
      throw new ApiError("VALIDATION_ERROR", "WhiteBIT mapping route IDs must be unique and included in the selected routes.", 400);
    }
    selected.set(entry.networkId, {
      assetCode: entry.assetCode.trim().toUpperCase(),
      networkCode: entry.networkCode.trim().toUpperCase(),
    });
  }
  return selected;
}

function isPlausibleWhitebitRouteSetup(
  capabilities: Awaited<ReturnType<typeof getWhitebitCapabilities>> | null,
  networkId: string,
  assetCode: string,
  networkCode: string,
  whitebitAssetCode: string | null | undefined,
  whitebitNetworkCode: string | null | undefined,
) {
  if (!capabilities) return false;
  const assetTicker = whitebitAssetCode?.trim().toUpperCase() || assetCode.trim().toUpperCase();
  const providerNetworks = capabilities.assets.find(asset => asset.ticker === assetTicker)?.depositNetworks ?? [];
  return isWhitebitRouteMappingPlausible(
    networkId, assetCode, networkCode, assetTicker, whitebitNetworkCode, providerNetworks,
  );
}
...
function reviewDepositProviderAssignments(
  ...
  mappings: ReadonlyMap<string, { assetCode: string; networkCode: string }> = new Map(),
) {
  ...
  const routes = sorted.map(({ asset, network, assetMapping }) => {
    ...
    const mappedAsset = proposed?.assetCode ?? network.whitebitAssetCode;
    const mappedNetwork = proposed?.networkCode ?? network.whitebitNetworkCode;
    const effectiveAsset = mappedAsset ?? asset.code;
    const validAssetIdentity = effectiveAsset.trim().toUpperCase() === asset.code.trim().toUpperCase() ||
      assetMapping?.providerTicker.trim().toUpperCase() === effectiveAsset.trim().toUpperCase();
    const advertisedNetworkOptions = target === "whitebit"
      ? capabilities?.assets.find(row => row.ticker === effectiveAsset.trim().toUpperCase())?.depositNetworks ?? []
      : [];
    const whitebitIdentityException = target === "whitebit" &&
      isWhitebitRouteIdentityException(network.id, asset.code, mappedAsset);
    const whitebitNetworkOptions = target === "whitebit" && !whitebitIdentityException
      ? plausibleWhitebitDepositNetworks(network.networkCode, advertisedNetworkOptions, effectiveAsset)
      : [];
    const capability = target === "whitebit" && capabilities && validAssetIdentity
      ? matchWhitebitRouteCapability(capabilities, asset.code, network.networkCode, mappedAsset, mappedNetwork) : null;
    const mappingPlausible = target === "whitebit" && !whitebitIdentityException
      ? isWhitebitRouteMappingPlausible(
          network.id, asset.code, network.networkCode, mappedAsset, mappedNetwork, advertisedNetworkOptions,
        )
      : false;
    const mappingStatus = target !== "whitebit" || !validAssetIdentity || whitebitIdentityException
      ? "unsupported" as const
      : capability && mappingPlausible ? "supported" as const
      : whitebitNetworkOptions.length ? "mapping_required" as const : "unsupported" as const;
    const currentWhitebitReady = target === "whitebit" && network.depositProvider === "whitebit" &&
      !proposed && eligibility && isCustomerDepositEligible(asset, network, eligibility);
    const walletReady = hasUsableSavedReceivingWallet(network);
    const status = target === "whitebit" && mappingStatus !== "supported" ? "unsupported" as const
    ...
    const reason = status === "unsupported"
      ? mappingStatus === "mapping_required"
        ? `Select a WhiteBIT network advertised for ${effectiveAsset}; the internal network ${network.networkCode} is not an exact provider match.`
        : !validAssetIdentity
          ? `WhiteBIT asset ${effectiveAsset} is not linked to canonical asset ${asset.code}.`
          : whitebitIdentityException
            ? "The Polygon route has immutable USDT0 identity and cannot use WhiteBIT USDT."
          : `WhiteBIT does not advertise deposits for asset ${effectiveAsset}.`
      : switchingWithDepositsEnabled
        ? "Turn off Customer Deposits before changing this route's provider or WhiteBIT mapping."
      : !active
    ...
    return {
      ...
      whitebitAssetCode: mappedAsset ?? null,
      whitebitNetworkCode: mappedNetwork ?? null,
  ...
  const reviewToken = createHash("sha256").update(JSON.stringify({
    ...
    rows: sorted.map(({ asset, network }) => ({
      ...
      whitebitAssetCode: network.whitebitAssetCode, whitebitNetworkCode: network.whitebitNetworkCode,
      manualWalletTrackingEnabled: network.manualWalletTrackingEnabled,
      ...
      mappedAssetIdentity: sorted.find(row => row.network.id === network.id)?.assetMapping?.providerTicker ?? null,
    })),
  })).digest("hex");
...
async function loadDepositProviderAssignmentPairs(
  ...
  const query = executor.select({ asset: cryptoAssetsTable, network: cryptoAssetNetworksTable, assetMapping: whitebitAssetMappingsTable })
    .from(cryptoAssetNetworksTable)
    .innerJoin(cryptoAssetsTable, eq(cryptoAssetsTable.id, cryptoAssetNetworksTable.assetId))
    .leftJoin(whitebitAssetMappingsTable, eq(whitebitAssetMappingsTable.assetId, cryptoAssetsTable.id))
    .where(inArray(cryptoAssetNetworksTable.id, networkIds));
  // A nullable mapping join cannot be locked with an unqualified FOR UPDATE.
  // Route writes are fenced by the provider advisory lock above.
  const pairs = lock ? await query.for("update", { of: cryptoAssetNetworksTable }) : await query;
...
router.post("/admin/crypto-networks/deposit-provider/preview", requireOwner, async (req, res, next) => {
  try {
    ...
    const mappings = selectedWhitebitMappings(input.whitebitMappings, input.networkIds, input.depositProvider);
    ...
    res.json(PreviewCryptoDepositProviderAssignmentResponse.parse(
      reviewDepositProviderAssignments(pairs, input.depositProvider, capabilities, eligibility, mappings),
    ));
  } catch (error) { next(error); }
});
...
router.post("/admin/crypto-networks/deposit-provider/apply", requireOwner, async (req, res, next) => {
  try {
    ...
    const result = await db.transaction(async tx => {
      ...
      const review = reviewDepositProviderAssignments(pairs, input.depositProvider, capabilities, eligibility, mappings);
      ...
      for (const { network, asset, assetMapping } of pairs) {
        const mapping = mappings.get(network.id);
        if (mapping && mapping.assetCode !== asset.code.trim().toUpperCase() &&
          mapping.assetCode !== assetMapping?.providerTicker.trim().toUpperCase()) {
          throw new ApiError("WHITEBIT_ASSET_MAPPING_REQUIRED", "A WhiteBIT asset ticker differing from the canonical asset needs a separately verified asset identity.", 422);
        }
        await tx.update(cryptoAssetNetworksTable).set({
          depositProvider: input.depositProvider,
          ...(mapping ? { whitebitAssetCode: mapping.assetCode, whitebitNetworkCode: mapping.networkCode } : {}),
...
async function loadWhitebitAutoMappingRows(
  executor: Pick<typeof db, "select">,
  lock: boolean,
) {
  const query = executor.select({ asset: cryptoAssetsTable, network: cryptoAssetNetworksTable })
    .from(cryptoAssetNetworksTable)
    .innerJoin(cryptoAssetsTable, eq(cryptoAssetsTable.id, cryptoAssetNetworksTable.assetId))
    .orderBy(asc(cryptoAssetsTable.code), asc(cryptoAssetNetworksTable.networkCode), asc(cryptoAssetNetworksTable.id));
  return lock ? query.for("update") : query;
}

function reviewWhitebitAutomaticMappings(
  pairs: Awaited<ReturnType<typeof loadWhitebitAutoMappingRows>>,
  catalog: Awaited<ReturnType<typeof fetchWhitebitCatalogAssets>>,
  catalogFetchedAt: Date,
) {
  const autoRoutes: WhitebitAutoMappingRoute[] = pairs.map(({ asset, network }) => ({
    networkId: network.id,
    assetCode: asset.code,
    networkCode: network.networkCode,
  ...
  const classified = classifyWhitebitAutoMappings(autoRoutes, catalog);
  const routes = classified.map((route, index) => ({
    ...route,
    whitebitAssetCode: pairs[index]?.network.whitebitAssetCode ?? null,
    whitebitNetworkCode: pairs[index]?.network.whitebitNetworkCode ?? null,
  }));
  const counts = {
    total: routes.length,
    automatic: routes.filter(route => route.status === "automatic").length,
    eligibleToApply: routes.filter(route => route.willApply).length,
    ownerSelection: routes.filter(route => route.status === "owner_selection").length,
    ambiguous: routes.filter(route => route.ambiguous).length,
    unsupported: routes.filter(route => route.status === "unsupported").length,
    existingMappings: routes.filter(route => route.status === "existing_mapping").length,
    protected: routes.filter(route => route.status === "protected").length,
  };
  const stableCatalog = catalog.map(asset => ({
    ticker: asset.normalizedTicker,
    canDeposit: asset.canDeposit,
    depositNetworks: asset.networks.filter(network => network.canDeposit)
      .map(network => network.providerNetwork.trim().toUpperCase()).sort(),
  })).sort((a, b) => a.ticker.localeCompare(b.ticker));
  const reviewToken = createHash("sha256").update(JSON.stringify({
    catalog: stableCatalog,
    routes: pairs.map(({ asset, network }) => ({ asset, network })),
  })).digest("hex");
  return { reviewToken, catalogFetchedAt, counts, updated: 0, routes };
}

async function fetchFreshWhitebitCatalog() {
  try {
    const catalog = await fetchWhitebitCatalogAssets();
    return { catalog, fetchedAt: new Date() };
  } catch {
    throw new ApiError("WHITEBIT_CATALOG_UNAVAILABLE", "WhiteBIT public catalog is unavailable; no route mapping can be reviewed or applied.", 503);
  }
}

router.post("/admin/crypto-networks/whitebit-auto-mapping/preview", requireOwner, async (req, res, next) => {
  try {
    PreviewWhitebitAutomaticRouteMappingsBody.parse(req.body);
    const { catalog, fetchedAt } = await fetchFreshWhitebitCatalog();
    const pairs = await loadWhitebitAutoMappingRows(db, false);
    res.json(PreviewWhitebitAutomaticRouteMappingsResponse.parse(
      reviewWhitebitAutomaticMappings(pairs, catalog, fetchedAt),
    ));
  } catch (error) { next(error); }
});

router.post("/admin/crypto-networks/whitebit-auto-mapping/apply", requireOwner, async (req, res, next) => {
  try {
    const input = ApplyWhitebitAutomaticRouteMappingsBody.parse(req.body);
    const { catalog, fetchedAt } = await fetchFreshWhitebitCatalog();
    const actor = res.locals.operator as OperatorAuthorization;
    const result = await db.transaction(async tx => {
      await tx.execute(sql`select pg_advisory_xact_lock(hashtextextended('rook:whitebit:credentials', 0))`);
      await tx.execute(sql`select pg_advisory_xact_lock(hashtext('whitebit-provider'))`);
      const pairs = await loadWhitebitAutoMappingRows(tx, true);
      const review = reviewWhitebitAutomaticMappings(pairs, catalog, fetchedAt);
      if (review.reviewToken !== input.reviewToken) {
      ...
      for (const route of review.routes) {
        ...
        await tx.update(cryptoAssetNetworksTable).set({
          ...
          eq(cryptoAssetNetworksTable.id, route.networkId),
          isNull(cryptoAssetNetworksTable.whitebitAssetCode),
          isNull(cryptoAssetNetworksTable.whitebitNetworkCode),
          eq(cryptoAssetNetworksTable.executionMode, "manual"),
          eq(cryptoAssetNetworksTable.enabled, true),
          eq(cryptoAssetNetworksTable.lifecycle, "active"),
        ));
      ...
      if (appliedIds.length) {
        ...
        await tx.insert(operatorAuditLogsTable).values({
          ...
          details: {
            ...
            routes: review.routes.filter(route => appliedIds.includes(route.networkId)).map(route => ({
              networkId: route.networkId,
              assetCode: route.suggestedAssetCode,
              networkCode: route.suggestedNetworkCode,
            })),
          },
        });
      }
      return ApplyWhitebitAutomaticRouteMappingsResponse.parse({
        ...review,
        updated: appliedIds.length,
      });
    });
...
router.post("/admin/crypto-assets/bulk/apply", requireOwner, async (req, res, next) => {
  try {
    ...
    const selectedNetworkRows = networkIds.length
      ? await db.select().from(cryptoAssetNetworksTable)
        .where(inArray(cryptoAssetNetworksTable.id, networkIds))
      : [];
    const selectedNetworkById = new Map(selectedNetworkRows.map(network => [network.id, network]));
    const assetEligibilityChanged = input.edits.some(edit =>
      edit.enabled !== undefined || edit.lifecycle !== undefined,
    );
    const networkProofRouteIds = networkEdits.flatMap(network => {
      const current = selectedNetworkById.get(network.networkId);
      return current && (
        network.enabled !== undefined && network.enabled !== current.enabled ||
        network.lifecycle !== undefined && network.lifecycle !== current.lifecycle ||
        network.requiresMemo !== undefined && network.requiresMemo !== current.requiresMemo
      ) ? [network.networkId] : [];
    });
    const routeEligibilityChanged = networkProofRouteIds.length > 0;
    const affectedEligibilityContext = assetEligibilityChanged || routeEligibilityChanged
      ? await createCustomerDepositEligibilityContext()
      : undefined;
    const result = await db.transaction(async (tx) => {
      await tx.execute(sql`select pg_advisory_xact_lock(hashtext('whitebit-provider'))`);
      if (affectedEligibilityContext) {
        await assertCustomerDepositEligibilityContextCurrent(tx, affectedEligibilityContext);
      }
      ...
      const affectedAssetRoutes = assetEligibilityChanged
        ? await tx.select({ id: cryptoAssetNetworksTable.id })
          .from(cryptoAssetNetworksTable)
          .where(inArray(cryptoAssetNetworksTable.assetId, assetIds))
          .for("update")
        : [];
      const invalidatedRouteIds = [
        ...networkProofRouteIds,
        ...affectedAssetRoutes.map(route => route.id),
      ];
      if (invalidatedRouteIds.length) {
        await invalidateWhitebitDepositRouteProofsForRoutes(
          tx,
          invalidatedRouteIds,
          affectedEligibilityContext,
        );
      }
      for (const edit of input.edits) {
        ...
        for (const networkEdit of edit.networks ?? []) {
          ...
          if (changesDepositEligibility) {
            ...
            networkChanges.customerDepositsEnabled = currentNetwork.customerDepositsEnabled && Boolean(
              nextAsset.enabled &&
...
router.patch("/admin/crypto-assets/:id", requireOperator, async (req, res, next) => {
  try {
    ...
    const eligibilityContext = invalidatesProviderProofs
      ? await createCustomerDepositEligibilityContext()
      : undefined;
    const row = await db.transaction(async (tx) => {
      ...
      if (eligibilityContext) {
        ...
        const affectedRoutes = await tx.select({ id: cryptoAssetNetworksTable.id })
          .from(cryptoAssetNetworksTable)
          .where(eq(cryptoAssetNetworksTable.assetId, id))
          .for("update");
        await invalidateWhitebitDepositRouteProofsForRoutes(
          tx,
          affectedRoutes.map(route => route.id),
          eligibilityContext,
        );
      }
  ...
  try {
    ...
    const row = await db.transaction(async (tx) => {
      ...
      await tx.update(cryptoAssetNetworksTable)
        ...
        .where(eq(cryptoAssetNetworksTable.assetId, id));
      return tx.delete(cryptoAssetsTable)
        .where(eq(cryptoAssetsTable.id, id))
        .returning()
...
router.put("/admin/crypto-assets/:id/receiving-wallet", requireOwner, async (req, res, next) => {
  try {
    ...
    if (
      input.customerDepositsEnabled !== undefined &&
      input.enabled !== undefined &&
      input.customerDepositsEnabled !== input.enabled
    ) {
      throw new ApiError(
        "VALIDATION_ERROR",
        "customerDepositsEnabled and the legacy enabled field must match.",
        400,
      );
    }
    const requestedCustomerDepositsEnabled =
      input.customerDepositsEnabled ?? input.enabled;
    const explicitCustomerDepositIntent = input.customerDepositsEnabled === true;
    const legacyEnabledIntent = input.customerDepositsEnabled === undefined &&
      input.enabled !== undefined;
    const actor = res.locals.operator as OperatorAuthorization;
    ...
    const selectedRoute = await db.select({
      ...
      manualWalletTrackingEnabled: cryptoAssetNetworksTable.manualWalletTrackingEnabled,
    ...
    const fallbackByRoute = await db.select({
      ...
      sharedDepositAddress: cryptoAssetNetworksTable.sharedDepositAddress,
      manualWalletTrackingEnabled: cryptoAssetNetworksTable.manualWalletTrackingEnabled,
      depositProvider: cryptoAssetNetworksTable.depositProvider,
    }).from(cryptoAssetNetworksTable)
      .where(inArray(cryptoAssetNetworksTable.id, affectedRouteIds));
    const readinessRoutes = affectedRouteIds.flatMap(routeId => {
      const fallback = fallbackByRoute.find(row => row.id === routeId);
      const trackingEnabled = routeId === selectedRoute[0].id
        ? input.manualWalletTrackingEnabled ?? fallback?.manualWalletTrackingEnabled
        : fallback?.manualWalletTrackingEnabled;
      const provider = routeId === selectedRoute[0].id
        ? input.depositProvider ?? fallback?.depositProvider
        : fallback?.depositProvider;
      const address = input.walletAddress.trim() || fallback?.sharedDepositAddress || "";
    ...
    const manualReadiness = readinessRoutes.length
      ? await prepareManualMonitoringReadiness(readinessRoutes)
    ...
    const rows = await db.transaction(async (tx) => {
      ...
      if (
        ...
        !isRegisteredDepositProvider(input.depositProvider)
      ) {
        throw new ApiError(
          "CRYPTO_DEPOSIT_PROVIDER_UNAVAILABLE",
          "The selected deposit provider is not registered.",
          422,
        );
      }
      ...
      const providerChanged = input.depositProvider !== undefined &&
        input.depositProvider !== selected.depositProvider;
      const effectiveEligibilityContext = providerChanged
        ? {
            ...eligibilityContext,
            whitebitProofs: new Map([...eligibilityContext.whitebitProofs]
              .filter(([routeId]) => routeId !== selected.id)),
          }
        : eligibilityContext;
      if (providerChanged) {
        await invalidateWhitebitDepositRouteProofsForRoutes(
      ...
      for (const network of affected) {
        ...
        const nextProvider = network.id === selected.id
          ? input.depositProvider ?? network.depositProvider
          : network.depositProvider;
        const manualWalletTrackingEnabled = network.id === selected.id &&
          input.manualWalletTrackingEnabled !== undefined
          ? input.manualWalletTrackingEnabled
        ...
        const routeAsset = assetById.get(network.assetId) ?? asset;
        if (nextProvider === "whitebit" && (
          isWhitebitRouteIdentityException(network.id, routeAsset.code, network.whitebitAssetCode) ||
          eligibilityContext.whitebitCapabilities && !isPlausibleWhitebitRouteSetup(
            eligibilityContext.whitebitCapabilities,
            network.id,
            routeAsset.code,
            network.networkCode,
            network.whitebitAssetCode,
          ...
          throw new ApiError("WHITEBIT_ROUTE_UNSUPPORTED", "This WhiteBIT route identity is not a plausible same-chain mapping for the selected Asset + Network.", 422);
        }
        const readiness = manualReadiness.get(network.id);
        if (manualWalletTrackingEnabled && readiness?.ready &&
        ...
        if (
          (explicitCustomerDepositIntent || nextProvider === "none") &&
          fallbackMemo &&
        ...
        if (explicitCustomerDepositIntent && fallbackAddress && network.requiresMemo && !fallbackMemo) {
          throw new ApiError(
            "CRYPTO_DEPOSIT_MEMO_REQUIRED",
            `A memo or tag is required for the ${network.networkCode} manual fallback wallet.`,
            422,
          );
        }
        const publicIntent = network.id === selected.id &&
          requestedCustomerDepositsEnabled !== undefined
          ? requestedCustomerDepositsEnabled
          : network.customerDepositsEnabled;
        if (explicitCustomerDepositIntent && publicIntent && nextProvider === "manual" && !fallbackAddress) {
          throw new ApiError(
            "CRYPTO_DEPOSIT_ADDRESS_REQUIRED",
        ...
        const eligible = isCustomerDepositEligible(
          ...
          assetById.get(network.assetId)?.enabled &&
          assetById.get(network.assetId)?.lifecycle !== "deprecated" &&
          network.enabled &&
        ...
        const nextEnabled = publicIntent &&
          eligible &&
          (!legacyEnabledIntent || !manualWalletTrackingEnabled || readiness?.ready !== false);
        await tx.update(cryptoAssetNetworksTable).set({
          sharedDepositAddress: fallbackAddress,
          sharedDepositMemo: fallbackMemo || null,
      ...
      await tx.insert(operatorAuditLogsTable).values({
        ...
        details: {
          ...
          providerAfter: input.depositProvider ?? selected.depositProvider,
          ...
          manualWalletTrackingBefore: selected.manualWalletTrackingEnabled,
          manualWalletTrackingAfter: updated.find(network => network.id === selected.id)?.manualWalletTrackingEnabled,
      ...
      req.log.info(
        {
          ...
          useForAllAssetsOnNetwork: input.useForAllAssetsOnNetwork,
          customerDepositsEnabled: requestedCustomerDepositsEnabled ?? "preserved",
        },
...
router.post("/admin/crypto-networks/receiving-wallet/preview", requireOwner, async (req, res, next) => {
  try {
    ...
    if (mappingProvided && (input.networkIds.length !== 1 ||
      ...
      throw new ApiError("VALIDATION_ERROR", "Choose both WhiteBIT asset and network for one exact route, or clear both.", 400);
    }
    if (
      input.customerDepositsEnabled !== undefined &&
      input.enabled !== undefined &&
      ...
      throw new ApiError("VALIDATION_ERROR", "customerDepositsEnabled and enabled must match.", 400);
    }
    const eligibilityContext = await createCustomerDepositEligibilityContext();
    ...
    const selectedAssets = await db.select().from(cryptoAssetsTable)
    ...
    for (const routeId of input.networkIds) {
      ...
      const provider = input.preserveDepositProviders
        ? network.depositProvider
        : input.depositProvider ?? network.depositProvider;
      const mappedAsset = mappingProvided ? input.whitebitAssetCode : network.whitebitAssetCode;
      if (provider === "whitebit" && isWhitebitRouteIdentityException(network.id, asset.code, mappedAsset)) {
        throw new ApiError("WHITEBIT_ROUTE_UNSUPPORTED", "The Polygon route has immutable USDT0 identity and cannot use WhiteBIT USDT.", 422);
      }
    }
    ...
    if (capabilities) {
      for (const routeId of input.networkIds) {
        ...
        const provider = input.preserveDepositProviders
          ? network.depositProvider
          : input.depositProvider ?? network.depositProvider;
        if (provider !== "whitebit" && !mappingProvided) continue;
        const mappedAsset = mappingProvided ? input.whitebitAssetCode : network.whitebitAssetCode;
        const mappedNetwork = mappingProvided ? input.whitebitNetworkCode : network.whitebitNetworkCode;
        if (!isPlausibleWhitebitRouteSetup(
          capabilities, network.id, asset.code, network.networkCode, mappedAsset, mappedNetwork,
        )) {
          throw new ApiError("WHITEBIT_ROUTE_UNSUPPORTED", `WhiteBIT mapping ${mappedAsset ?? asset.code} · ${mappedNetwork ?? network.networkCode} is not a plausible same-chain route for ${asset.code} · ${network.networkCode}.`, 422);
        }
      }
    }
    const rows = input.networkIds.map(routeId => {
      ...
      const customerDepositsEnabled = input.customerDepositsEnabled ??
        input.enabled ?? network.customerDepositsEnabled;
      const prospectiveNetwork = {
        ...network,
        enabled: networkEnabled,
        depositProvider: provider,
        whitebitAssetCode: mappingProvided ? input.whitebitAssetCode ?? null : network.whitebitAssetCode,
      ...
      if (customerDepositsEnabled) {
        if (
          ...
          readiness = {
            ...
            message: "Customer Deposits require a valid wallet or exact verified provider route.",
            ready: false,
            networkCode: network.networkCode,
          };
        }
...
router.put("/admin/crypto-networks/receiving-wallet", requireOwner, async (req, res, next) => {
  try {
    ...
    const selectedAssetsForReadiness = await db.select()
      .from(cryptoAssetsTable)
      .where(inArray(cryptoAssetsTable.id, [...new Set(selectedForReadiness.map(row => row.assetId))]));
    const assetByIdForReadiness = new Map(selectedAssetsForReadiness.map(asset => [asset.id, asset]));
    for (const network of selectedForReadiness) {
      const asset = assetByIdForReadiness.get(network.assetId);
      const provider = input.preserveDepositProviders
        ? network.depositProvider
        : input.depositProvider ?? network.depositProvider;
      const mappedAsset = mappingProvided ? input.whitebitAssetCode : network.whitebitAssetCode;
      if (asset && provider === "whitebit" &&
        isWhitebitRouteIdentityException(network.id, asset.code, mappedAsset)) {
        throw new ApiError("WHITEBIT_ROUTE_UNSUPPORTED", "The Polygon route has immutable USDT0 identity and cannot use WhiteBIT USDT.", 422);
      }
    }
    const enablingWhitebit = (requestedCustomerDepositsEnabled === true || mappingProvided) &&
    ...
    const whitebitCapabilities = enablingWhitebit
      ...
      : null;
    if (whitebitCapabilities) {
      for (const network of selectedForReadiness) {
        const asset = assetByIdForReadiness.get(network.assetId);
        const provider = input.preserveDepositProviders
          ? network.depositProvider
          : input.depositProvider ?? network.depositProvider;
        if (!asset || (provider !== "whitebit" && !mappingProvided)) continue;
        const mappedAsset = mappingProvided ? input.whitebitAssetCode : network.whitebitAssetCode;
        const mappedNetwork = mappingProvided ? input.whitebitNetworkCode : network.whitebitNetworkCode;
        if (!isPlausibleWhitebitRouteSetup(
          whitebitCapabilities, network.id, asset.code, network.networkCode, mappedAsset, mappedNetwork,
        )) {
          throw new ApiError("WHITEBIT_ROUTE_UNSUPPORTED", `WhiteBIT mapping ${mappedAsset ?? asset.code} · ${mappedNetwork ?? network.networkCode} is not a plausible same-chain route for ${asset.code} · ${network.networkCode}.`, 422);
        }
      }
    ...
    const rows = await db.transaction(async (tx) => {
      ...
      const effectiveEligibilityContext = proofInvalidatedIds.length
        ? {
             ...eligibilityContext,
             whitebitProofs: new Map([...eligibilityContext.whitebitProofs]
               .filter(([routeId]) => !proofInvalidatedIds.includes(routeId))),
          }
        : eligibilityContext;
      ...
      for (const network of selected) {
        ...
        const networkEnabled = input.networkEnabled ?? network.enabled;
         const nextProvider = input.preserveDepositProviders
           ? network.depositProvider
           : input.depositProvider ?? network.depositProvider;
          const customerDepositsEnabled = requestedCustomerDepositsEnabled ??
            network.customerDepositsEnabled;
        const nextNetwork = {
          ...network,
          enabled: networkEnabled,
         ...
         let readiness = manualReadiness.get(network.id);
          ...
          if (nextProvider !== "none" && readiness?.ready &&
            readiness.code !== "LEGACY_BEP20") {
          const [currentMonitor] = await tx.select().from(blockchainMonitorNetworksTable)
            .where(eq(blockchainMonitorNetworksTable.id, readiness.monitorNetworkId!))
            .for("update")
        ...
        const asset = assetById.get(network.assetId)!;
        if (mappingProvided && nextProvider !== "whitebit") {
          throw new ApiError("VALIDATION_ERROR", "WhiteBIT mapping can only be edited while WhiteBIT is the selected provider.", 400);
        }
        if (mappingProvided && nextNetwork.whitebitAssetCode &&
          nextNetwork.whitebitAssetCode.trim().toUpperCase() !== asset.code.trim().toUpperCase()) {
          const [approved] = await tx.select({ ticker: whitebitAssetMappingsTable.providerTicker })
            .from(whitebitAssetMappingsTable)
            .where(eq(whitebitAssetMappingsTable.assetId, asset.id)).limit(1);
          if (nextNetwork.whitebitAssetCode.trim().toUpperCase() !== approved?.ticker.trim().toUpperCase()) {
            throw new ApiError("WHITEBIT_ASSET_MAPPING_REQUIRED", "The selected WhiteBIT asset is not linked to this canonical asset.", 422);
          }
        }
        if (mappingProvided && nextNetwork.whitebitAssetCode && whitebitCapabilities &&
          !matchWhitebitRouteCapability(whitebitCapabilities, asset.code, network.networkCode,
            nextNetwork.whitebitAssetCode, nextNetwork.whitebitNetworkCode)) {
          throw new ApiError("WHITEBIT_ROUTE_UNSUPPORTED", `WhiteBIT does not advertise deposits for ${nextNetwork.whitebitAssetCode} · ${nextNetwork.whitebitNetworkCode}. No deposit settings were saved.`, 422);
        }
        if (
          customerDepositsEnabled &&
        ...
        await tx.update(cryptoAssetNetworksTable).set({
          enabled: networkEnabled,
           depositProvider: nextProvider,
          ...(mappingProvided
            ? { whitebitAssetCode: nextNetwork.whitebitAssetCode, whitebitNetworkCode: nextNetwork.whitebitNetworkCode }
            : {}),
          sharedDepositAddress: address,
          sharedDepositMemo: memo || null,
...
router.patch("/admin/crypto-networks/:id/customer-deposits", requireOwner, async (req, res, next) => {
  try {
    ...
    const eligibilityContext = input.enabled && current.depositProvider === "whitebit"
      ? await createCustomerDepositEligibilityContext()
      : undefined;
    const readiness = input.enabled &&
      current.depositProvider === "manual" &&
      current.manualWalletTrackingEnabled
    ...
    const updated = await db.transaction(async (tx) => {
      ...
      if (input.enabled && (
        ...
        locked.depositProvider === "none"
      )) {
        throw new ApiError(
          "CRYPTO_DEPOSIT_ROUTE_BLOCKED",
          "This exact Asset + Network route must be enabled, active, use Manual execution, and have a deposit provider selected.",
          409,
        );
      }
      const [asset] = await tx.select().from(cryptoAssetsTable)
        ...
        .limit(1);
      if (!asset) {
        throw new ApiError("CRYPTO_ASSET_NOT_FOUND", "Crypto asset not found.", 404);
      }
      if (input.enabled && (
        !asset.enabled ||
        asset.lifecycle === "deprecated"
        ...
        throw new ApiError(
          "CRYPTO_DEPOSIT_ROUTE_BLOCKED",
          "The exact Asset + Network route must belong to an enabled, non-deprecated asset.",
          409,
        );
      }
      ...
      if (input.enabled && !isCustomerDepositEligible(asset, locked, eligibility)) {
        throw new ApiError(
          "CRYPTO_DEPOSIT_VERIFICATION_REQUIRED",
...
router.patch("/admin/crypto-networks/:id", requireOperator, async (req, res, next) => {
  try {
    ...
    const changesDepositEligibility = depositEligibilityKeys.some((key) =>
      Object.hasOwn(input, key) && input[key] !== current[key]
    );
    const invalidatesProviderProofs =
      changesDepositEligibility &&
      (current.depositProvider === "whitebit" || input.depositProvider === "whitebit");
    const eligibilityContext = invalidatesProviderProofs
      ? await createCustomerDepositEligibilityContext()
      : undefined;
    const row = await db.transaction(async (tx) => {
      ...
      if (
        Object.hasOwn(input, "depositProvider") &&
        input.depositProvider !== locked.depositProvider
      ) {
        throw new ApiError(
          "OWNER_REQUIRED",
          "Deposit provider assignments must be changed through the Owner receiving-wallet workflow.",
          403,
        );
      }
      if (
        Object.hasOwn(input, "manualWalletTrackingEnabled") &&
        input.manualWalletTrackingEnabled !== locked.manualWalletTrackingEnabled
      ) {
        throw new ApiError(
          "OWNER_REQUIRED",
          "Manual Wallet Tracking must be changed through the Owner receiving-wallet workflow.",
          403,
        );
      }
      if (
        Object.hasOwn(input, "sharedDepositAddress") &&
        input.sharedDepositAddress !== locked.sharedDepositAddress ||
        Object.hasOwn(input, "sharedDepositMemo") &&
        (input.sharedDepositMemo ?? null) !== locked.sharedDepositMemo
      ) {
        throw new ApiError(
          "OWNER_REQUIRED",
          "Receiving wallet addresses and memos must be changed through the Owner receiving-wallet workflow.",
          403,
        );
      }
      if (
        Object.hasOwn(input, "sharedDepositAddress") &&
        input.sharedDepositAddress &&
        !isSyntacticallyValidManualWalletAddress(
          { ...locked, ...input },
          String(input.sharedDepositAddress),
        )
      ) {
        throw new ApiError(
          "CRYPTO_DEPOSIT_ADDRESS_INVALID",
          `The receiving address is invalid for ${locked.networkCode}.`,
          422,
        );
      }
      if (
        Object.hasOwn(input, "sharedDepositMemo") &&
        input.sharedDepositMemo &&
        !isSyntacticallyValidManualWalletMemo(
          { ...locked, ...input },
          String(input.sharedDepositMemo),
        )
      ) {
        throw new ApiError(
          "CRYPTO_DEPOSIT_MEMO_INVALID",
          `The receiving memo or tag is invalid for ${locked.networkCode}.`,
          422,
        );
      }
      if (eligibilityContext) {
        await invalidateWhitebitDepositRouteProofsForRoutes(tx, [id], eligibilityContext);
      }
      ...
      const [asset] = await tx.select().from(cryptoAssetsTable)
        ...
        .limit(1);
      if (!asset) throw new ApiError("CRYPTO_ASSET_NOT_FOUND", "Crypto asset not found.", 404);
      const effectiveEligibilityContext = eligibilityContext
        ? {
            ...eligibilityContext,
            whitebitProofs: new Map([...eligibilityContext.whitebitProofs]
      ...
      const derivedCustomerDepositsEnabled = changesDepositEligibility
        ? locked.customerDepositsEnabled &&
          isCustomerDepositEligible(asset, nextNetwork, effectiveEligibilityContext)
        : locked.customerDepositsEnabled;
...
router.delete("/admin/crypto-networks/:id", requireOperator, async (req, res, next) => {
  try {
    ...
    const row = await db.transaction(async (tx) => {
      await tx.execute(sql`select pg_advisory_xact_lock(hashtext('whitebit-provider'))`);
      await invalidateWhitebitDepositRouteProofsForRoutes(tx, [id]);
      return tx.delete(cryptoAssetNetworksTable)
        .where(eq(cryptoAssetNetworksTable.id, id))
        .returning()
        .then(([deleted]) => deleted);
    });
    if (!row) throw new ApiError("CRYPTO_NETWORK_NOT_FOUND", "Crypto network not found.", 404);
    res.sendStatus(204);
  } catch (e) { next(e); }
});
...
router.get("/admin/providers/whitebit/verification-routes", requireOwner, async (_req, res, next) => {
  try {
    ...
    res.json(rows.filter(({ asset, network }) =>
      isPlausibleWhitebitRouteSetup(
        capabilities, network.id, asset.code, network.networkCode,
        network.whitebitAssetCode, network.whitebitNetworkCode,
      ) &&
      matchWhitebitRouteCapability(capabilities, asset.code, network.networkCode, network.whitebitAssetCode, network.whitebitNetworkCode)
    ).map(({ asset, network }) => ({
      networkId: network.id,
      assetCode: asset.code,
      networkCode: network.networkCode,
...
router.post("/admin/providers/whitebit/address-permission/verify", requireOwner, async (req, res, next) => {
  try {
    ...
    if (!selected ||
      !isPlausibleWhitebitRouteSetup(
        capabilities, selected.network.id, selected.asset.code, selected.network.networkCode,
        selected.network.whitebitAssetCode, selected.network.whitebitNetworkCode,
      ) ||
      !matchWhitebitRouteCapability(capabilities, selected.asset.code, selected.network.networkCode,
        selected.network.whitebitAssetCode, selected.network.whitebitNetworkCode)) {
    ...
    const outcome = await db.transaction(async (tx) => {
      ...
      if (!currentRoute || !isPlausibleWhitebitRouteSetup(
        capabilities,
        currentRoute.network.id,
  ...
  try {
    ...
    if (req.body.enabled && publicCapabilities) {
      ...
      const exactCandidates = capturedConfigured.filter((row) =>
        isPlausibleWhitebitRouteSetup(
          publicCapabilities, row.network.id, row.asset.code, row.network.networkCode,
          row.network.whitebitAssetCode, row.network.whitebitNetworkCode,
        ) &&
        matchWhitebitRouteCapability(
          publicCapabilities,
          row.asset.code,
          row.network.networkCode,
          row.network.whitebitAssetCode,
          row.network.whitebitNetworkCode,
        )
      );
      ...
      const verifiedKeys = new Set(verifiedProofs.flatMap((proof) => {
        const candidate = candidateById.get(proof.networkId)!;
        const capability = matchWhitebitRouteCapability(
          publicCapabilities, candidate.asset.code, candidate.network.networkCode,
          candidate.network.whitebitAssetCode, candidate.network.whitebitNetworkCode,
        );
        return capability ? [`${capability.providerTicker}/${capability.providerNetwork}`] : [];
      }));
