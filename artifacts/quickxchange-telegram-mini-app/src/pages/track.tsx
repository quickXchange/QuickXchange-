...
import { getTelegramMiniAppOrder } from '@workspace/api-client-react';
...
export default function Track() {
  ...
  const handleTrack = async (e: React.FormEvent) => {
    ...
    try {
      const order = await getTelegramMiniAppOrder(orderId.trim(), { headers });

      if (order) {
        haptic.notification('success');
        setLocation(`/orders/${orderId.trim()}`);
      }
    } catch (err: any) {
