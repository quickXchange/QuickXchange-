...
export function QuickexConvertWidget({
  ...
  const emptyRouteRefreshRef = useRef('');
  ...
  useEffect(() => {
    ...
    if (reqSource && reqDest) {
      ...
      if (pair) {
        const sourceKey = instrumentKey(pair.fromAsset, pair.fromNetwork);
        const targetKey = instrumentKey(pair.toAsset, pair.toNetwork);
        ...
        if (fromSlug !== sourceInst.slug) {
          setFromSlug(sourceInst.slug);
          return;
        }
        const targetInst = pairOptions.find(item =>
          instrumentKey(item.currencyTitle, item.networkTitle) === targetKey
        );
        if (!targetInst) return;
        if (toSlug !== targetInst.slug) {
          setToSlug(targetInst.slug);
        }
      } else {
              ...
              <div className="order-details-content convert-order-details-content flex flex-col gap-4">
                ...
                <div className="order-detail-field order-detail-field--refund flex flex-col gap-1.5">
                  <label htmlFor="convert-refund" className="text-[13px] font-semibold text-muted-foreground">
                    {t('convert.refundAddress')} · {from?.currencyTitle} <small className="font-normal">({t('convert.optional')})</small>
                  </label>
                  <div className="relative">
