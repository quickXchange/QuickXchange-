...
const [ethereumMigrationSql, ethereumRangeMigrationSql, bscRpcMigrationSql, bscRangeMigrationSql] =
  await Promise.all([
...
const exactBscConfigurationQuery = (secretReference, maxScanRange) => `
  SELECT 1
...
const bscImmutableFingerprintQuery = `
  SELECT
    (
      SELECT md5(coalesce(string_agg(row_to_json(identity)::text, E'\\n' ORDER BY id), ''))
      FROM blockchain_monitor_assets identity
...
try {
  ...
  if (existingBsc.rowCount + existingBscV2.rowCount !== 1) {
    throw new Error(
  ...
  const verifiedBsc = await client.query(
    exactBscConfigurationQuery("BSC_MONITOR_RPC_URL_V2", 9),
  );
  if (verifiedBsc.rowCount !== 1) {
    throw new Error(
      "Production monitoring migration did not preserve the exact BNB and USDT identities on the BSC V2 monitor.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Production monitoring migration changed BSC identities, watches, cursors, observations, or matches.",
    );
  }
