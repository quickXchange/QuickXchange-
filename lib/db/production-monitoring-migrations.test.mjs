...
async function createFixture(client, schema, { malformedBscIdentity = false } = {}) {
  ...
  await client.query(`
    ...
    (
      'mon-trc20', 'TRC20', 'TRON', 'evm', NULL, 'rpc',
      true, NULL, 19, 'confirmations',
      15, 1000, NULL, NULL, 'not_configured', NULL, 0
    ),
    (
      'mon-polygon', 'POLYGON', 'Polygon PoS', 'evm', NULL, 'rpc',
      true, NULL, 1, 'confirmations',
      30, 1000, NULL, NULL, 'not_configured', NULL, 0
    )
  `);
  await client.query(
    `INSERT INTO blockchain_monitor_assets (
     ...
     VALUES
       ...
       ($4, 'mon-trc20', 'trx-tron', 'native', NULL, 6, true),
       ($5, 'mon-trc20', 'usdt-trc20', 'native', NULL, 6, true),
       ($6, 'mon-polygon', 'pol-polygon', 'native', NULL, 18, true)`,
    [
      randomUUID(),
  ...
  await client.query(`
    INSERT INTO crypto_asset_networks (
      id, asset_id, network_code, decimals, shared_deposit_address,
      customer_deposits_enabled
    )
    VALUES
      ('bnb-bep20', 'bnb', 'BEP20', 18, NULL, true),
      ('usdt-bep20', 'usdt', 'BEP20', 6, NULL, true),
      ('usdc-bep20', 'usdc', 'BEP20', 6, NULL, true),
      ('eth-ethereum', 'eth', 'ERC20', 18, NULL, true),
      ('usdt-erc20', 'usdt', 'ERC20', 6, NULL, true),
      ('usdc-erc20', 'usdc', 'ERC20', 6, NULL, true),
      ('trx-tron', 'trx', 'TRC20', 6, 'TBLc145ZDNs4LjPqQtuvqkEDjhesemTosd', true),
      ('usdt-trc20', 'usdt', 'TRC20', 6, 'TBLc145ZDNs4LjPqQtuvqkEDjhesemTosd', true),
      ('pol-polygon', 'pol', 'POLYGON', 18, '0x961fFd69412BdD402B8a63FC67D5d7f1A105abDb', false);

    INSERT INTO crypto_assets (id)
    VALUES ('trx'), ('usdt'), ('pol')
  `);
}
...
test("production monitoring migration is concurrent-safe, idempotent, and preserves operational history", async () => {
  await withFixture({}, async ({ client, schema }) => {
    ...
    const polygon = await client.query(`
      SELECT
        ...
        last_head,
        health_status
      FROM ${schema}.blockchain_monitor_networks
      WHERE id = 'mon-polygon'
    `);
    assert.deepEqual(polygon.rows[0], {
      id: "mon-polygon",
      chain_id: "0x89",
    ...
    await client.query(`
      ...
      SET
        endpoint_secret_ref = NULL,
        cursor = '86482880',
        last_head = '86482899',
        health_status = 'connected',
        health_checked_at = '2026-09-22T23:50:17.016Z',
        health_error = NULL,
        consecutive_failures = 0,
        health_proof_fingerprint = 'existing-network-proof',
        health_proof_captured_at = '2026-09-22T23:50:17.016Z'
      WHERE network_code = 'TRC20'
    `);
    await client.query(`
      UPDATE blockchain_monitor_assets
      SET
        readiness_proof_fingerprint = 'existing-route-proof',
        readiness_proof_captured_at = '2026-09-22T23:50:17.016Z'
      WHERE monitor_network_id = 'mon-trc20'
    `);
    const beforeEndpointRepair = await client.query(`
      SELECT
        cursor,
        last_head,
        health_status,
        ...
        (
          SELECT jsonb_agg(
            jsonb_build_object(
              'id', identity.id,
              'proof', identity.readiness_proof_fingerprint,
              'capturedAt', identity.readiness_proof_captured_at
            )
      ...
      WHERE network_code = 'TRC20'
    `);
    const endpointRepair = await runMigration(schema);
    assert.equal(endpointRepair.code, 0, endpointRepair.stderr || endpointRepair.stdout);
    const afterEndpointRepair = await client.query(`
      SELECT
        endpoint_secret_ref,
        cursor,
        last_head,
      ...
      WHERE network_code = 'TRC20'
    `);
    const { endpoint_secret_ref: repairedEndpoint, ...preservedAfterRepair } =
      afterEndpointRepair.rows[0];
    assert.equal(repairedEndpoint, "TRON_MONITOR_API_URL");
    assert.deepEqual(preservedAfterRepair, beforeEndpointRepair.rows[0]);
    ...
    const configuration = await client.query(`
      SELECT
        ...
        (SELECT count(*)::int FROM blockchain_monitor_assets
         WHERE monitor_network_id = 'monitor-bep20'
           AND asset_network_id IN ('usdt-bep20','usdc-bep20')
           AND identity_kind = 'token'
           AND decimals = 18)
          AS verified_bsc_stablecoin_identities,
        ...
        (SELECT endpoint_secret_ref FROM blockchain_monitor_networks WHERE network_code = 'TRC20')
          AS tron_secret_ref,
        (SELECT adapter_kind FROM blockchain_monitor_networks WHERE network_code = 'TRC20')
          AS tron_adapter,
        (SELECT provider_kind FROM blockchain_monitor_networks WHERE network_code = 'TRC20')
          AS tron_provider,
        (SELECT count(*)::int FROM blockchain_monitor_assets identity
         JOIN blockchain_monitor_networks monitor ON monitor.id = identity.monitor_network_id
         WHERE monitor.network_code = 'TRC20'
           AND identity.enabled = true
           AND (
             (identity.asset_network_id = 'trx-tron'
              AND identity.identity_kind = 'native'
              AND identity.contract_or_mint IS NULL
              AND identity.decimals = 6)
             OR
             (identity.asset_network_id = 'usdt-trc20'
              AND identity.identity_kind = 'token'
              AND identity.contract_or_mint =
                '41a614f803b6fd780986a42c78ec9c7f77e6ded13c'
              AND identity.decimals = 6)
           ))
          AS verified_tron_identities,
        (SELECT count(*)::int FROM crypto_asset_networks
         WHERE id IN ('trx-tron','usdt-trc20')
           AND customer_deposits_enabled = false)
          AS disabled_tron_routes
    `);
    assert.deepEqual(configuration.rows[0], {
      ...
      verified_bsc_stablecoin_identities: 2,
      disabled_ethereum_routes: 3,
      tron_secret_ref: "TRON_MONITOR_API_URL",
      tron_adapter: "tron",
      tron_provider: "indexer",
      verified_tron_identities: 2,
      disabled_tron_routes: 2,
    });
  });
});
